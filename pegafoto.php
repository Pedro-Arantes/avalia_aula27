...
try {
    ...
    if ($_POST) {
      if ($_POST["idproduto"]) {
        $sql = "UPDATE tblprodutos set nome=:nome,imagem=:imagem,valor=:valor,estoque=:estoque,estoquemax=:estoquemax,estoquemin=:estoquemin where idproduto=:idproduto ";
        $stmt = $con->prepare($sql);
        ...
        $sql = "INSERT into tblprodutos(nome,valor,estoque,estoquemax,estoquemin) values(:nome,:valor,:estoque,:estoquemax,:estoquemin)";
        $stmt = $con->prepare($sql);
        $stmt->bindValue(":nome",$_POST["nome"]);
        $stmt->bindValue(":valor",$_POST["valor"]);
        $stmt->bindValue(":estoque",$_POST["estoque"]);
...
<body>
    ...
    <h1>Formulário de Cadastro de Produtos</h1>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="<?php echo isset($produto) ? $produto->estoquemin : null ?>" name="estoquemin">
